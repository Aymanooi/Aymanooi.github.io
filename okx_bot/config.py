...
# === رافعة متدرّجة حسب الرصيد (طلب المستخدم) ===
# سببان للرافعة الأعلى عند الرصيد الصغير:
#  1) عملي: برصيد $5 ورافعة x2 حجم الصفقة $9.5 فقط، وكثير من عملات OKX لها حدّ
#     أدنى ($5–10) فتُرفض. x5 يرفع الحجم إلى ~$23 فتُفتح عملات أكثر.
#  2) تسريع المرحلة الأولى — مقابل خطر أعلى (x5 فوق Kelly، فالتراجع أعمق).
# بعد تجاوز العتبة نعود إلى x2 (الرافعة المثلى رياضياً = أعلى نمو هندسي).
LEV_TIERS = [
    (20.0, 5),   # رصيد < $20  → رافعة x5 (مرحلة الانطلاق)
    (1e18, 2),   # رصيد ≥ $20  → رافعة x2 (Kelly المثلى)
]

def leverage_for_balance(balance):
    """يُرجع الرافعة المناسبة حسب حجم الرصيد (متدرّجة تنازلياً مع النمو)."""
    for ceiling, lev in LEV_TIERS:
        if balance < ceiling:
            return lev
    return LEV_TIERS[-1][1]
