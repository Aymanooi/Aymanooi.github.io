...
STOP_LOSS_PCT   = 0.005   # وقف الخسارة: 0.5% — أمثل على فريم 1m (PF=1.07)
TAKE_PROFIT_PCT = 0.015   # هدف الربح: 1.5% — نسبة ربح/خسارة 3:1
...
# === حماية من التصفية (طلب «لا تتصفّى») ===
# التصفية تقع عند حركة معاكسة ≈ 1/الرافعة. لمنعها نحدّ الرافعة بحيث تحتاج
# التصفية فجوة سعر ≥ LIQ_GAP_BUFFER. القيمة 0.06 = لا تصفية إلا بفجوة 6%
# (تسقُف الرافعة فعلياً عند ~x16). أصغر = رافعة أعلى لكن حماية أقل.
LIQ_GAP_BUFFER = 0.06
