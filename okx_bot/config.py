...
STOP_LOSS_PCT   = 0.005   # وقف الخسارة: 0.5%
TAKE_PROFIT_PCT = 0.005   # هدف الربح: 0.5% — نسبة 1:1 (طلب المستخدم؛ البيانات: PF 0.60 خسارة)
...
USE_MAX_LEVERAGE = True    # أقصى رافعة لكل عملة (طلب المستخدم الصريح، يتحمّل المسؤولية الكاملة)

# لا تدخل إلا العملات التي رافعتها القصوى أكبر من هذه القيمة (طلب المستخدم: > x50)
MIN_COIN_MAX_LEVERAGE = 50

# === حماية من التصفية ===
# 0.0 = معطّل (رافعة قصوى حقيقية بلا سقف — طلب المستخدم، يتحمّل المسؤولية).
