...
# لا تدخل إلا العملات التي رافعتها القصوى أكبر من هذه القيمة.
# 29 (طلب المستخدم: x30 فما فوق فقط) — يُتخطّى أي عملة رافعتها القصوى
# ≤ 29، فلا تدخل إلا التي تدعم x30+ (فتصل رافعتك الكاملة بلا قصّ).
MIN_COIN_MAX_LEVERAGE = int(os.getenv("BOT_MIN_COIN_MAX_LEV", "29"))
...
# 0.02 = سقف فعلي x50 (طلب المستخدم) — يسمح برافعة x50 الكاملة.
# ⚠️ عند x50: التصفية تقع على حركة معاكسة ~2% فقط.
LIQ_GAP_BUFFER = float(os.getenv("BOT_LIQ_GAP_BUFFER", "0.02"))
